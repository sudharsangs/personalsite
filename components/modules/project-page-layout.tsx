...
const ProjectPageLayout: React.FC<ProjectPageLayoutProps> = ({
  ...
  const { icon: TypeIcon, label, badgeBg, badgeText, badgeBorder, accentBar } = typeConfig[type];
    ...
    <div className="min-h-screen">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-primary/5 via-background to-accent/8 border-b border-border/50">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          {/* Grid pattern */}
          <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'linear-gradient(hsl(220 70% 50%) 1px, transparent 1px), linear-gradient(90deg, hsl(220 70% 50%) 1px, transparent 1px)', backgroundSize: '48px 48px' }} />
        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10">
          ...
          <div className="mb-10">
            <Link href="/projects">
              <Button variant="ghost" className="text-muted-foreground hover:text-foreground hover:bg-secondary/80 -ml-2 gap-2">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className={`inline-flex items-center gap-1.5 ${badgeBg} ${badgeText} border ${badgeBorder} px-3 py-1 rounded-full text-xs font-semibold`}>
            ...
            {company && (
              ...
              >
                ...
                />
                <span className="text-sm text-foreground/80">{company.name}</span>
          ...
          {/* Accent bar */}
          <div className={`h-1.5 w-32 rounded-full bg-gradient-to-r ${accentBar} mb-6 shadow-sm`} />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 relative">
        ...
        <div className="flex justify-center pt-8 mt-8 border-t border-border/40">
          <Link href="/projects" className="inline-flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors text-sm">
...
export const ProjectSection: React.FC<{
  ...
  <div className={className}>
    <div className="flex items-center gap-3 mb-8">
      <div className={`${iconBg} p-2.5 rounded-xl border ${iconBorder} shadow-sm`}>
        <div className={iconColor}>{icon}</div>
      </div>
      <div>
        <h2 className="text-2xl font-bold text-foreground" style={{ fontFamily: fonts.PlayfairDisplay }}>
          {title}
        </h2>
...
export const FeatureList: React.FC<{
  ...
  <ul className="grid sm:grid-cols-2 gap-3">
    {items.map((item, index) => (
      <li key={index} className="flex items-start gap-3 group bg-white/60 hover:bg-white/90 border border-border/50 hover:border-primary/25 rounded-xl px-4 py-3 transition-all duration-200 shadow-sm hover:shadow-md">
        <span className={`flex-shrink-0 inline-flex items-center justify-center h-6 w-6 rounded-full ${iconBg} ${iconColor} mt-0.5 shadow-sm`}>
...
export const InfoCard: React.FC<{
  ...
  const variantStyles = {
    ...
    destructive: 'bg-destructive/5 border-destructive/15 [&_h3]:text-destructive',
...
export const StatCard: React.FC<{
  ...
  <div className="bg-white rounded-2xl border border-border/60 shadow-md hover:shadow-lg hover:border-primary/30 p-6 text-center transition-all duration-200 relative overflow-hidden group">
    ...
    <div className="relative z-10">
      <div className="text-3xl font-bold bg-gradient-to-br from-primary to-primary/70 bg-clip-text text-transparent mb-1">{number}</div>
